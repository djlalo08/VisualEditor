...
class LabelModal(tk.Toplevel):
    def __init__(self, root, ins, outs, fn_name='', labels=None) -> None:
        ...
        self.all_vars = [self.center, self.top, self.bot, self.left,
                         self.right] + self.in_tops + self.in_btwns + self.in_bots + self.out_tops + self.out_btwns + self.out_bots
    ...
    def entry(self, var, x, y, w=t_w, h=t_h, incl_lr_pad=True, incl_tb_pad=True):
        ...
        e = tk.Entry(self, textvariable=var, justify='center', background='#F8F2DE', highlightcolor='#F8F2DE',
                     highlightbackground='#E7B680', foreground='black')
